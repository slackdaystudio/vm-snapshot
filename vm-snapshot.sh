...
print_help () {
    ...
    printf "Usage:\n  -v [VM_NAMES]\n  -o [PATH]\n\n"
    printf "Example: vm-snapshot -v vm1,vm2,vm3 -o /tmp/backups\n"
}
...
if [ ${#VMs[@]} -eq 0 ] || [ ! -d $BACKUP_DIR  ]; then
    ...
    printf "(vm-snapshot -v vm1 -o /tmp/backups).\n\n"
    printf "Multiple VMs may be passed with commas (-v vm1,vm2,etc...).\n"
...
for name in ${VMs[@]}; do
    ...
    ${BACKUP_TOOL}${START_VM} --noprogress -d ${name} -l auto -o ${SNAPSHOT_PATH}
